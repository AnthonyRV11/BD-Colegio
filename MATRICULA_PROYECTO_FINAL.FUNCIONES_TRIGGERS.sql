...
-------------------------------------------------------------
/*7 TRIGGER*/
-- TRIGGER PARA CONVIRTIR EN NULL EL PROFESOR EN UN MÓDULO ABIERTO CUANDO EL PROFESOR SE BORRA
GO
CREATE TRIGGER BORRADO_PROFESOR_MA
ON PROFESORES AFTER DELETE
AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @ID_PROFESOR VARCHAR(20);
    SELECT @ID_PROFESOR = ID_PROFESOR FROM deleted;

    UPDATE MODULOS_ABIERTOS SET ID_PROFESOR = NULL WHERE ID_PROFESOR = @ID_PROFESOR;
END;
GO

-- TRIGGER PARA GUARDAR EL HISTORIAL DE PROFESORES BORRADOS
GO
...
-- TRIGGER PARA GUARDAR EL HISTORIAL DE ESTUDIANTES BORRADOS
GO
...
BEGIN
    ...
    VALUES(GETDATE(), @CARNET_E, 'ESTUDIANTE BORRADO', SYSTEM_USER);
END;
GO

-- TRIGGER PARA ELIMINAR UN MODULO ABIERTO CUANDO EL MÓDULO SE BORRA
GO
...
-- TRIGGER PARA ELIMINAR UN HORARIO AL BORRAR UN GRUPO
GO
...
BEGIN
    ...
    DELETE HORARIOS WHERE ID_GRUPO = @COD_GRUPO;
END;
GO

-- TRIGGER PARA ELIMINAR EL DETALLE DE MATRÍCULA AL BORRAR UNA MATRÍCULA
GO
...
-- TRIGGER PARA PONER EN NULL EL LABORATORIO EN UN MÓDULO ABIERTO AL SER BORRADO
GO
CREATE TRIGGER BORRADO_LAB_MA
ON LABORATORIOS AFTER DELETE
AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @ID_LAB VARCHAR(15);
    SELECT @ID_LAB = ID_LAB FROM deleted;
...
(
    @CARNET_E VARCHAR(20),
    @COD_MODULO_A VARCHAR(30),
    @MSJ VARCHAR(100) OUTPUT,
    @NUM_RECIBO INT OUTPUT,
    @ESTADO VARCHAR(5),
...
BEGIN
    BEGIN TRY
        ...
        DECLARE @EXISTENCIA_E INT, @EXISTENCIA_MA INT, @NOTA_ANTERIOR INT, @CUPOS INT

        SET @EXISTENCIA_E = DBO.BUSCAR_ESTUDIANTE(@CARNET_E)
        SET @EXISTENCIA_MA = DBO.BUSCAR_MODULO_A(@COD_MODULO_A)
        SET @NOTA_ANTERIOR = DBO.COMPROBANDO_APROBADO(@CARNET_E)
        SET @CUPOS = DBO.COMPROBANDO_CUPOS_MODULO(@COD_MODULO)

        IF (@EXISTENCIA_E = 1 AND @EXISTENCIA_MA = 1 AND @NOTA_ANTERIOR = 1 AND @CUPOS = 1)
        BEGIN
...
GO
CREATE PROCEDURE SP_ABRIR_MODULO
(
    @COD_MODULO_A VARCHAR(30),
...
BEGIN
    BEGIN TRY
        ...
        DECLARE @PROFESOR_C INT, @EXISTE_LAB INT, @CHOQUE_H INT

        SET @PROFESOR_C = DBO.COMPROBANDO_PROF(@COD_CER, @ID_PROFESOR);
        ...
        BEGIN
            ...
            VALUES(@ID_GRUPO, @COD_MODULO_A, @NOMBRE_GRUPO, @CUPOS);
...
-- Función para verificar la fecha en la que termina un módulo abierto
GO
CREATE FUNCTION AVERIGUANDO_FECHA_FINAL
(
    @FECHA_INICIO DATE,
    @TOTAL_DIAS INT,
    @HORA_I TIME,
...
BEGIN
    ...
    SET @FECHA_FINAL = DATEADD(DAY, @TOTAL_DIAS, @FECHA_FINAL);
    RETURN @FECHA_FINAL;
END;
GO